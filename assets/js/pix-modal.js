class PixPaymentModal {
  ...
  async generatePix() {
    ...
    try {
      ...
      this.renderQRCode(pixData.qrCodeBase64);
  ...
  renderQRCode(qrCodeBase64) {
    const container = document.querySelector('.pix-qrcode-wrapper');
    ...
    if (!qrCodeBase64) {
      throw new Error('QR Code Base64 não disponível');
    }

    const img = document.createElement('img');
    img.src = `data:image/png;base64,${qrCodeBase64}`;
    img.className = 'pix-qrcode-canvas';
